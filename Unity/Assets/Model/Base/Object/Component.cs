...
namespace ETModel
{
	...
	public abstract class Component : Object, IDisposable
	{
...
#if !SERVER
		public static GameObject Global { get; } = GameObject.Find("/[GlobalInit]/ET");
		...
		public virtual void Dispose()
		{
			...
			// 触发Destroy事件
			ETGame.EventSystem.Destroy(this);

			ETGame.EventSystem.Remove(this.InstanceId);
			
			this.InstanceId = 0;

			if (this.IsFromPool)
			{
				ETGame.ObjectPool.Recycle(this);
			}
			else
			{
#if !SERVER
		...
		{
			ETGame.EventSystem.Deserialize(this);
		}
