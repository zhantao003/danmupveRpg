...
[CDanmuGiftAttrite(CDanmuGiftConst.Soldier_Lv1)]
public class DGiftSoldier_Lv1 : CDanmuGiftAction
{
    ...
    void CreateUnit(CPlayerBaseInfo player, long num, long price, bool joinPlayer)
    {
        ...
        if (CGameAntGlobalMgr.Ins.emGameType == CGameAntGlobalMgr.EMGameType.LocalPvP)
        {
            if (CSceneMgr.Instance.m_objCurScene.emSceneType == CSceneFactory.EMSceneType.GameMap101)
            {
                ...
                pLSEvent.msgParams.SetLong("num", num * CGameAntGlobalMgr.Ins.pStaticConfig.GetInt("GiftSolider6"));

                CLockStepMgr.Ins.AddLSEvent(pLSEvent);
                //送礼动画
                if (player.emCamp == EMUnitCamp.Red)
                {
                    UIScreenInfoInsertionMgr.Ins.SetAInsertion(InfoInsertionType.左送精英怪, new IIPlayerSendGift(player, CDanmuGiftConst.Soldier_Lv1, pTBLInfo.szName, (int)num, 0));
                }
                else
                {
                    UIScreenInfoInsertionMgr.Ins.SetAInsertion(InfoInsertionType.右送精英怪, new IIPlayerSendGift(player, CDanmuGiftConst.Soldier_Lv1, pTBLInfo.szName, (int)num, 0));
                }
            }
        }
        else if (CGameAntGlobalMgr.Ins.emGameType == CGameAntGlobalMgr.EMGameType.NetPvP)
        {
            ...
            pEventCreateSoldier.Num = num * CGameAntGlobalMgr.Ins.pStaticConfig.GetInt("GiftSolider6");
